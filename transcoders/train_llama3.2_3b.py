...
from train_gpt2 import compute_expert_count
...
def compute_expert_count_mlp(multiplier, d_in=3072, d_out=3072, moe_expansion_factor=2.67):
    ...
    d_sae_moe = int(d_in * moe_expansion_factor)
...
def compute_expert_count_glu(multiplier, d_in=3072, d_out=3072, d_sae_new=8192):
    # Compute d_sae for the transcoder
    d_sae_transcoder = d_in * multiplier

    # Total parameters for the transcoder
    total_transcoder = d_sae_transcoder * (d_in + 1 + d_out) + d_out

    # New layer parameters that do NOT depend on N:
    const_new_layer = 2 * d_sae_new * (d_in + 1) + d_out * (d_sae_new + 1)

    # The factor that multiplies N in the new layer:
    N_factor = d_in + 1 + d_out

    # Solve for N such that total_new_layer_params equals total_transcoder_params:
    N = (total_transcoder - const_new_layer) / N_factor
    return int(N)
...
def parse_args():
    ...
    parser.add_argument("--d_out", type=int, default=3072,
    ...
    parser.add_argument("--expansion_factor", type=int, default=2.67,
...
def main():
    ...
    if args.use_conditional:
        if args.use_glu:
            ...
            n_experts = compute_expert_count_glu(args.expert_multiplier)
            print('USING GLU!')
        else:
            # Compute the number of experts based on the provided multiplier.
            n_experts = compute_expert_count_mlp(args.expert_multiplier)
