    ...
    for i in tqdm.tqdm(range(0, tokens_arr.shape[0], batch_size)):
        with torch.no_grad():
            ...
            if use_raw_scores:
                ...
                _, hidden_acts, _, _, _, _, _ = encoder(mlp_acts_flattened)
...
def make_sequence_html(token_strs, scores,
    ...
    tokens_html = "".join([
        f"""<span class='token'
            style='background-color: {color_strs[i]}; color: black;'
...
def display_activating_examples_dash(model, all_tokens, scores,
    ...
    for band in reversed(unif_bands):
        cur_html_list = [f"<details open><summary><{header_tag} style='display: inline; font-family: serif'>Between {band[0]:.2f} and {band[1]:.2f}: {100*band[2]:.4f}%</{header_tag}></summary>"]
