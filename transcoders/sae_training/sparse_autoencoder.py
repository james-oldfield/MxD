...
class SparseAutoencoder(HookedRootModule):
    ...
    def __init__(
        ...
        self.randomize_k = False
        
        self.use_conditional = cfg.use_conditional
        self.use_kaiming_init = cfg.use_kaiming_init
        self.use_transpose_decoder_init = cfg.use_transpose_decoder_init
        self.use_glu = cfg.use_glu
        self.use_skip = cfg.use_skip
        self.n_experts = cfg.n_experts
        self.topk = cfg.topk
        self.norm_dec = cfg.norm_dec
        self.topk_aux_loss = cfg.topk_aux_loss

        self.training_perc = 0
        self.current_exp_usage = None

        # set the activation function for the hidden units (last one being for GLUs)
        if cfg.mlp_activation_fn == "relu":
            self.mlp_activation_fn = F.relu
        elif cfg.mlp_activation_fn == "gelu":
            self.mlp_activation_fn = F.gelu
        elif cfg.mlp_activation_fn == "sigmoid":
            self.mlp_activation_fn = F.sigmoid
        elif cfg.mlp_activation_fn == "gelu_pytorch_tanh":
            self.mlp_activation_fn = partial(F.gelu, approximate='tanh')
        else:
            raise ValueError(f"Unexpected activation function: {cfg.mlp_activation_fn}")
        ...
        # standard linear layer init by default
        if not self.use_kaiming_init or self.use_conditional:
            self.W_dec.data = nn.Linear(self.d_sae, self.d_out, device=self.device).weight.data.T
            self.W_enc.data = nn.Linear(self.d_in, self.d_sae, device=self.device).weight.data.T

        # any additional GLU params
        if self.use_glu:
            self.W_glu = nn.Linear(self.d_in, self.d_sae, device=self.device)

        if not self.use_conditional and self.use_transpose_decoder_init:
            print('WARN: using T init for transcoder')
            self.W_dec.data = self.W_enc.data.T

        # skip transcoder connection
        if self.use_skip:
            self.W_skip = nn.Linear(self.d_in, self.d_in, device=self.device, bias=False)

            # zero-out init the TC skip connection and decoder weights
            self.W_skip.weight.data.fill_(0)
            self.W_dec.data.fill_(0)
            
        if self.use_conditional:
            self.gate = torch.nn.Parameter(torch.zeros(self.n_experts, self.d_in, device=self.device), requires_grad=True) # gating params
            self.gate_bias = torch.nn.Parameter(torch.zeros(self.n_experts, device=self.device), requires_grad=True)
            self.W_cond = nn.Parameter(torch.randn(self.d_out, self.n_experts, device=self.device), requires_grad=True)

            self.gate.data = nn.Linear(self.d_in, self.n_experts, device=self.device).weight.data
            self.W_cond.data = nn.Linear(self.n_experts, self.d_out, device=self.device).weight.data

            # following skip transcoders, initialize W_dec as the 0 matrix.
            self.W_dec.data.fill_(0)

            if self.cfg.norm_dec:
                torch.nn.init.uniform_(self.W_cond, -1, 1)
                self.set_decoder_norm_to_unit_norm()

            if self.use_transpose_decoder_init:
                print('WARN: using T init')
                self.gate.data = self.W_cond.data.T
            
        if self.cfg.norm_dec and not self.use_conditional:
            self.set_decoder_norm_to_unit_norm()
    ...
    def encode(self, x):
        ...
        if self.use_glu:
            # generate the hidden units via a GLU 
            feature_acts_pre = self.hook_hidden_post(self.mlp_activation_fn(hidden_pre))
            feature_acts_pre = self.W_glu(x) * feature_acts_pre
        else:
            # standard MLP linear transformation + non-linearity
            feature_acts_pre = self.hook_hidden_post(self.mlp_activation_fn(hidden_pre))
            
        if self.cfg.use_conditional or self.cfg.topk < 0:
            feature_acts = feature_acts_pre
        else:
            # topk gating on the hidden activations (transcoders/SAEs)
            topk = torch.topk(feature_acts_pre, k=self.cfg.topk, dim=-1)
            values = topk.values
            feature_acts = torch.zeros_like(feature_acts_pre)
            feature_acts.scatter_(-1, topk.indices, values)

            # optional hook for steering
            if self.a_hook is not None:
                feature_acts = self.a_hook(feature_acts, self)

        return feature_acts, feature_acts_pre
    ...
    def decode(self, feature_acts):
        ...
        # NOTE: no bias term is applied yet, so we can optionally condition
        return sae_out

    # perform the conditional branch
    def condition(self, sae_in):
        experts_pre = einops.einsum(sae_in, self.gate, '... i, o i -> ... o') + self.gate_bias
        experts_post = torch.relu(experts_pre)

        # topk (or l1 loss) on the *expert coefficients*
        if self.topk > 0:
            k = self.topk
            if self.training and self.randomize_k:
                # randomly sample topK (NOTE: disabled by default)
                offset = np.random.randint(-self.topk//2, self.topk//2 + 1)
                k = self.topk + offset
            else:
                k = self.topk

            topk = torch.topk(experts_post, k=k, dim=-1)
            values = topk.values
            indices = topk.indices
            expert_coefs = torch.zeros_like(experts_post)
            expert_coefs.scatter_(-1, indices, values)

        else:
            expert_coefs = experts_post
            
        # optional steering hook
        if self.a_hook is not None:
            expert_coefs = self.a_hook(expert_coefs, self)

        #################### (C@a) conditional branch in the paper
        expert_out = einops.einsum(
            self.W_cond,
            expert_coefs,
            'd_out n_experts, ... n_experts -> ... d_out'
        )

        return expert_out, expert_coefs, experts_pre
    ...
    def forward(self, x, dead_neuron_mask = None, mse_target=None):
        ...
        sae_out_pre = self.decode(feature_acts)

        if self.use_conditional:
            expert_out, expert_coefs, experts_pre = self.condition(x)
            sae_out = sae_out_pre * expert_out # elementwise product between MLP output and conditional branch (=> MoD)
        else:
            expert_coefs = None
            sae_out = sae_out_pre

        if self.use_skip and self.cfg.is_transcoder:
            sae_out = sae_out + self.W_skip(x)

        sae_out = self.hook_sae_out(sae_out + self.b_dec_out) if self.cfg.is_transcoder else self.hook_sae_out(sae_out + self.b_dec)

        if mse_target is None:
            # sae self-reconstruction
            mse_loss = (torch.pow((sae_out-x.float()), 2) / (x**2).sum(dim=-1, keepdim=True).sqrt())
        else:
            # sparse mlp / MoD reconstruction of MLP layers' outputs
        ...
        # top-k overwrites any specified l1 sparsity
        if self.cfg.topk > 0:
            l1_loss = torch.tensor(0.0, dtype=self.dtype, device=self.device)
            topk_loss = torch.tensor(-1.0, dtype=self.dtype, device=self.device) # NOTE: not implemented
            loss = mse_loss + topk_loss

        else:
            if self.cfg.norm_dec:
                # default SAE sparsity loss
                sparsity = torch.abs(feature_acts).sum(dim=1).mean(dim=(0,)) if not self.cfg.use_conditional else torch.abs(expert_coefs).sum(dim=1).mean(dim=(0,))
                l1_loss = self.l1_coefficient * sparsity
            else:
                if self.cfg.use_conditional:
                    weighted_feature_acts = expert_coefs * self.W_cond.T.norm(dim=1)
                else:
                    weighted_feature_acts = feature_acts * self.W_dec.norm(dim=1)
                sparsity = weighted_feature_acts.norm(p=1, dim=-1)  # sum over the feature dimension
                l1_loss = (self.l1_coefficient * sparsity).mean()
            topk_loss = torch.tensor(0.0, dtype=self.dtype, device=self.device)

            loss = mse_loss + l1_loss + mse_loss_ghost_resid

        if self.use_conditional:
            # return the expert coeficients in place of the feature_acts
            return sae_out, expert_coefs, loss, mse_loss, l1_loss, topk_loss, mse_loss_ghost_resid
        else:
            return sae_out, feature_acts, loss, mse_loss, l1_loss, topk_loss, mse_loss_ghost_resid
    ...
    def set_decoder_norm_to_unit_norm(self):
        ...
        if self.cfg.use_conditional:
            # note that factor is transpose of W_dec, so use dim=0.
            self.W_cond.data /= torch.norm(self.W_cond.data, dim=0, keepdim=True)
        else:
    ...
    def remove_gradient_parallel_to_decoder_directions(self):
        ...
        Update grads so that they remove the parallel component
            (d_sae, d_in) shape
        '''
        if self.cfg.use_conditional and self.cfg.norm_dec:
            ...
            self.W_cond.grad -= einops.einsum(
                parallel_component,
                self.W_cond.data,
                "N, d_out N -> d_out N",
            )
        else:
            # if not self.cfg.use_conditional:
            parallel_component = einops.einsum(
                self.W_dec.grad,
                self.W_dec.data,
    ...
    def get_name(self):
        sae_name = f"{self.cfg.custom_name + '_' if self.cfg.custom_name else ''}{self.cfg.model_name.split('/')[-1]}_{self.cfg.hook_point}-saewidth_{self.cfg.d_sae}-conditional_{self.cfg.use_conditional}-nexperts_{self.cfg.n_experts}-activfn_{self.cfg.mlp_activation_fn}-glu_{self.cfg.use_glu}-topk_{self.cfg.topk}-lr_{self.cfg.lr}_totaltokens_{self.cfg.total_training_tokens}"
        return sae_name