...
def train_sae_on_language_model(
    ...
    # use latents or experts?
    d_sae = sparse_autoencoder.cfg.d_sae if not sparse_autoencoder.cfg.use_conditional else sparse_autoencoder.cfg.n_experts
    
    # track active features
    act_freq_scores = torch.zeros(d_sae, device=sparse_autoencoder.cfg.device)
    n_forward_passes_since_fired = torch.zeros(d_sae, device=sparse_autoencoder.cfg.device)
    n_frac_active_tokens = 0
    
    optimizer = AdamW(sparse_autoencoder.parameters(),
    ...
    model.eval() # turn off dropout etc (learning recon mapping)

    # calculate number of params with numel sum
    num_params = sum(p.numel() for p in sparse_autoencoder.parameters())
    ...
    if use_wandb:
        sparse_autoencoder.cfg.num_params = num_params
        wandb.init(project=sparse_autoencoder.cfg.wandb_project, name=sparse_autoencoder.get_name(), config=sparse_autoencoder.cfg.__dict__)
    ...
    while n_training_tokens < total_training_tokens:
        ...
        sparse_autoencoder.training_perc = n_training_tokens / total_training_tokens
        
        # Do a training step.
        sparse_autoencoder.train()
        # Make sure the W_dec is still zero-norm
        if sparse_autoencoder.cfg.norm_dec:
        ...
        if not sparse_autoencoder.cfg.is_transcoder:
            ...
            sae_out, feature_acts, loss, mse_loss, l1_loss, topk_loss, ghost_grad_loss = sparse_autoencoder(
                sae_in,
                ...
                wandb.log(
                    {
                        ...
                        "losses/l1_loss": l1_loss.item() / sparse_autoencoder.l1_coefficient if sparse_autoencoder.l1_coefficient > 0 else -1 , # normalize by l1 coefficient
                        "losses/topk_loss": topk_loss.item(),
...
def run_evals(sparse_autoencoder: SparseAutoencoder, activation_store: ActivationsStore, model: HookedTransformer, n_training_steps: int):
    ...
    sae_out, feature_acts, _, _, _, _, _ = sparse_autoencoder(
        original_act
    )
